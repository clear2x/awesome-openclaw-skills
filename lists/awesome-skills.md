...
> 以下条目已纳入路线图，仓库先给中文导读；推荐前仍应继续核对其一手来源、依赖与维护状态。
...
- [待核对][中] `clawhub`：面向 ClawHub 的安装、发现、发布或索引浏览能力。
- [待核对][低] `coding-agent`：更偏“编码代理工作流封装”，适合把常见研发步骤打包。
- [待核对][中] `discord`：适合 Discord 频道、消息和社群协作自动化，但要严格控制外发行为。
- [待核对][低] `feishu-doc`：看名字更像飞书文档读取/理解入口，可补足 fetch/update/create 之外的通用导读。
- [待核对][中] `feishu-drive`：围绕飞书云盘文件的读取、查找、移动或组织工作流。
- [待核对][中] `feishu-perm`：用于管理飞书文档或文件权限，属于高价值但需高透明度的协作 skill。
- [待核对][中] `feishu-wiki`：围绕飞书知识库 / wiki 的读取、导航、页面整理与结构操作。
...
- [待核对][中] `notion`：更偏通用 Notion 集成入口，适合页面、数据库与知识组织。
...
## E. 待核对：知识、搜索、内容与多媒体候选

- [待核对][中] `canvas`：名称常见于白板、教育或内容画布类工作流，适合继续核对具体实现方向。
- [待核对][低] `gemini`：看名字像另一类模型或平台集成入口，适合做多模型协作补充。
...
- 下一步最值得做的是：给 51 个待核对条目补一手来源和依赖，继续把“路线图”变成“真正精选”。
